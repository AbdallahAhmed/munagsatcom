...
class Analysis
{
    ...
    public function check_double_focus_keyword($job, &$results)
    {
        ...
        if ($posts == 0) {
            ...
            $this->save_score_result($results, 6, trans('seo::seo.keyword_overused_6', ['opentag' => '<a href="' . url(ADMIN . $prop . '&kw_filter=' . $job['keyword']) . '" target="_blank">', 'endtag' => '</a>']), 'keyword_overused');
        } else {
            $this->save_score_result($results, 1, trans('seo::seo.keyword_overused_1', ['opentag' => '<a href="' . url(ADMIN . $prop . '&kw_filter=' . $job['keyword']) . '" target="_blank">', 'endtag' => '</a>', 'times' => $posts, 'url' => '<a href="https://yoast.com/cornerstone-content-rank/">']), 'keyword_overused');
        }
    }
    ...
    public function score_title($job, &$results)
    {
        ...
        if ($job['title'] == '') {
            ...
            $length = Text::textLength($job['title']);
    ...
    public function score_description($job, &$results, $description, $maxlength = 155)
    {
        ...
        if ($description == '') {
            ...
            $length = Text::textLength($description);
    ...
    public function score_body($job, &$results, $body, $firstp)
    {
        ...
        $keywordWordCount = Text::wordCount($job['keyword']);
...
//    }
    ...
    public function score_url($job, &$results)
    {
        ...
        if ((Text::textLength($job['keyword']) + 20) < Text::textLength($job['pageSlug']) && 40 < Text::textLength($job['pageSlug'])) {
